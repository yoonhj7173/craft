...
log = logging.getLogger("app.config_store")
...
_DEFAULTS = {
    ...
    "stripe_prices": (
        ...
        '"pro":"price_1TjvBuDPr15sURg1LBDafpNh",'
        '"studio":"price_1TjvBvDPr15sURg1lDvJAaLJ",'
        '"pack_s":"price_1TjvBvDPr15sURg1pd3M5brW",'
        '"pack_m":"price_1TjvBvDPr15sURg14rzhYn16",'
        '"pack_l":"price_1TjvBwDPr15sURg1fAXKGVTK"}'
    ),
}
...
def cost_usd(cfg: GuardConfig, model: str, tokens_in: int, tokens_out: int) -> float:
    ...
    if not p:
        # 가격 미설정 모델이면 0이 아니라 보수적 기본 단가로 추정한다 — 0을 반환하면 모든 task가
        # est_cost=0으로 기록돼 daily_cost_cap이 영영 안 걸린다(= 비용 무한, 감사 P0).
        log.warning("no model_pricing for %r — using default per-1k estimate for cost cap", model)
        return settings.estimate_cost_usd(tokens_in, tokens_out)
    return round(
